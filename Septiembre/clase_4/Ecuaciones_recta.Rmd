...
A dicho producto, definido sólo para $\mathbb{R}^3$ se le llama el **producto** **vectorial** de los vectores $\vec{u}$ y $\vec{v}$. Si $\vec{u}=(u_1,u_2,u_3)$ y $\vec{v}=(v_1,v_2,v_3)$
...
y $\begin{vmatrix} a & b \\ c & d \end{vmatrix}=ad-bc$

**Nota** Los vectores están dados por $\mathbf{i}:=(1,0,0)$, $\mathbf{j}:=(0,1,0)$ y $\mathbf{k}:=(0,0,1)$. Se les suele llamar _la_ _base_ _canónica_.

### Ejemplo

Calcular $\mathbf{u}\times \mathbf{v}$ y $\mathbf{v} \times \mathbf{u}$ si $\mathbf{u}=2\mathbf{i}+ 4\mathbf{j}- 5\mathbf{k}$ y $\mathbf{v}=-3\mathbf{i}-2\mathbf{j}+\mathbf{k}$.
...
* $\vec{u}\times(\vec{v}+\vec{w})=\vec{u}\times\vec{v}+\vec{w} \times \vec{u}$

* $\vec{u}$ y $\vec{v}$ son **ortogonales** a $\vec{u}\times \vec{v}$
