...
La recta real $\mathbb{R}$ es el conjunto de números reales.  Es el campo de escalares más típico.
...
Un **campo** **de** **escalares** es un conjunto $\mathbb{K}$ de elementos con las siguientes propiedades. 

Existen dos operaciones $+$ y $\cdot$, _suma_ y _multiplicación_ en el mismo conjunto tal que para $\alpha,\beta\in \mathbb{K}$
...
1. **Los racionales** $\mathbb{Q}$ con la suma y multiplicación de fracciones.

2. **Los números reales** $\mathbb{R}$ con la suma y multiplicación de números reales.

3 **Los números complejos** $\mathbb{C}$ con la suma y multiplicación de números complejos.

En el campo de escalares podemos multiplicar y dividir entre elementos del mismo campo.



Un vector dado con dichas coordenadas se considera que representa al vector $\vec{OP}$ que une el origen con el punto en el espacio $P$. Un vector dado $\vec{v}$ se puede representar de múltiples formas, anclado al origen o anclado a otro punto. Dos vectores son **equipotentes** ó **equipolentes** si son no nulos, tienen el mismo módulo, la misma dirección y sentido, es decir si representan al mismo vector, aunque su origen sea diferente.


![Vectores equipotentes](vectores-equipolentes.gif){ width=40% }		

Los vectores $\overrightarrow{AB}$ y $\overrightarrow{CD}$ son equipolentes por que tienen misma magnitud, dirección y sentido. 

En estas secciones nos centraremos en las definiciones y operaciones principales para $\mathbb{R}^{2}$, $\mathbb{R}^{3}$ y $\mathbb{R}^n$.
...
Sea $n$ un número entero positivo. Denotamos por $\mathbb{R}^n$ al conjunto de todas las $n-$tuplas de números reales. Es decir,

\begin{equation}
  \mathbb{R}^n =\Big\{ (x_1,x_2,\ldots,x_n)\,\,:\,\, x_i\in\mathbb{R} \mbox{ para }i=1,2,\ldots,n\Big\}
\end{equation}

## Definición de $\mathbb{K}^n$

Sea $n$ un número entero positivo. Denotamos por $\mathbb{K}^n$ al conjunto de todas las $n-$tuplas de números del campo. Es decir,

\begin{equation}
  \mathbb{K}^n =\Big\{ (x_1,x_2,\ldots,x_n)\,\,:\,\, x_i\in\mathbb{K} \mbox{ para }i=1,2,\ldots,n\Big\}
\end{equation}



A los números $x_i$ de la n-tupla $(x_1,x_2,\ldots,x_n)$ le llamamos la **i-ésima coordenada**.
...
$$
Para $\vec{a},\vec{b} \in \mathbb{R}^n$ entonces
\[
\vec{a} = \begin{pmatrix} a_1 \\ a_2 \\ \vdots \\ a_n \end{pmatrix} 
\quad \mbox{ y } \quad
\vec{b} = \begin{pmatrix} b_1 \\ b_2 \\ \vdots \\ b_n \end{pmatrix} \mbox{ entonces  }\quad 
...
y en general en $\mathbb{R}^n$, si $\vec{\mathtt{v}}=(\mathtt{v}_1,\mathtt{v}_2,\ldots,\mathtt{v}_n)$ y $\vec{\mathtt{u}}=(\mathtt{u}_1,\mathtt{u}_2,\ldots,\mathtt{u}_n)$

\[
\vec{\mathtt{v}} \cdot  \vec{\mathtt{u}} = v_1u_1 +v_2u_2 + \ldots + v_nu_n = \sum_{i=1}^n v_ku_k
\]
...
Suponga que $\vec{x},\vec{y}\in\mathbb{R}^n$ y $\alpha$ un escalar, $\alpha\in\mathbb{R}$.

@. Para todo $\vec{x}\in \mathbb{R}^n$, $\|\vec{x}\| \geq 0$.

@. $\|\alpha \cdot \vec{x}\| = |\alpha|\|\vec{x}\|$ para todo $\alpha\in \mathbb{R}$ y $\vec{x} \in \mathbb{R}^n$

@. Si $\|\vec{x}\|=0$ entonces $\vec{x}=\vec{0}$.

@. Para todo $\vec{x},\vec{y}\in \mathbb{R}^n$  se cumple **la Desigualdad de Cauchy-Schwarz**:
\begin{equation}
|\vec{x} \boldsymbol{\cdot} \vec{y}| \leq \|\vec{x}\| \|\vec{y}\|
\end{equation}

@. Para todo $\vec{x},\vec{y}\in \mathbb{R}^n$
\begin{equation}
\| \vec{x} + \vec{y} \| \leqslant \| \vec{x} \| + \| \vec{y} \|
\end{equation}

La última desigualdad recibe el nombre de **desigualdad** **del** **triángulo**.

Demostración:

Para 1) se tiene que $\| \vec{x} \| = \sqrt{x_1^2+x_2^2+\ldots + x_n^2}$ como $\vec{x} \boldsymbol{\cdot} \vec{x} \geq 0$ por propiedades del producto punto entonces $\| \vec{x} \| \geq 0$.

Para 2) por propiedades del producto punto 
\[
\|\alpha \cdot \vec{x}\|= 
\sqrt{ (\alpha\vec{x}) \boldsymbol{\cdot} (\alpha\vec{x})} = 
\sqrt{\alpha^2 \vec{x} \boldsymbol{\cdot} \vec{x}} = 
|\alpha|\sqrt{\vec{x} \boldsymbol{\cdot}  \vec{x} } =
|\alpha|\| \vec{x} \|
\]

Para 3), es obvio que si $\|\vec{x}\|=0$ entonces
\[
\sum_{i=1}^n x^2_i = 0 \Rightarrow x_i = 0
\]
...
\[
\begin{array}{lcl}
\| \vec{x} + \vec{y} \|^2  & = & (\vec{x} + \vec{y}) \boldsymbol{\cdot} (\vec{x} + \vec{y})  \\
\| \vec{x} + \vec{y} \|^2  & = & \vec{x} \boldsymbol{\cdot} (\vec{x} + \vec{y})   + \vec{y}  \boldsymbol{\cdot} (\vec{x} + \vec{y})  \\
\| \vec{x} + \vec{y} \|^2  & = & \vec{x} \boldsymbol{\cdot} \vec{x}    + \vec{x} \boldsymbol{\cdot} \vec{y}+ \vec{y}  \boldsymbol{\cdot} \vec{x} + \vec{y}  \boldsymbol{\cdot}\vec{y}  \\
\| \vec{x} + \vec{y} \|^2  & = & \| \vec{x} \|^2 + 2 \vec{x} \boldsymbol{\cdot} \vec{y}+ \| \vec{y} \|^2 \\
\| \vec{x}  + \vec{y} \|^2  & \leq & \| \vec{x} \|^2 + 2 \|\vec{x}\| \|\vec{y}\| + \| \vec{y} \|^2 = (\| \vec{x} \| + \|\vec{y} \|)^2 
\end{array}
\]
La última desigualdad proviene de la desigualdad de Cauchy-Schwarz aplicándola al segundo término, aplicando la raíz cuadrada tenemos
...
Si el ángulo entre dos vectores $\vec{x}$ y $\vec{y}$ es $90°$ (o $\frac{\pi}{2}$, $-\frac{\pi}{2}$) entonces
\[
\vec{x}\boldsymbol{\cdot} \vec{y} = \|\vec{x}\| \|\vec{y}\| \cos( \pm\pi/2)=0
\]
...
Llamamos a los vectores $\{\mathbf{i}=(1,0,0), \mathbf{j}=(0,1,0), \mathbf{k}=(0,0,1)\}$
...
1. Encontrar la norma de los siguientes vectores

* $P=(3,6)$  
* $A=(-1,3,4,1)$
* $\vec{OP}=(4,-1,3,5)$

2. Encontrar las siguientes normas, si $A=(3,-1,4)$ y $B=(4,2,-1)$
...
3. Encontrar la longitud del triángulo formado por 
