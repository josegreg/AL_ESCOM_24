...
1.- El producto de $a_{i_11}a_{i_22}\cdots a_{i_nn}$ aparece en el determinante de orden $n$ con el signo igual a $signo(\sigma)$, con $\sigma = \begin{pmatrix} 1 & 2 & 3 & \dots & n \\ i_1 & i_2 & i_3 & \dots & i_n \end{pmatrix}$

2.- Si se cambian dos filas o dos columnas de un determinante, el determinante sólo cambia de signo.
...
7.- Si todo elemento de la $k-$esima columna de un determinante viene dado como la suma de dos vectores columna $a_{ik}=b_{ik}+c_{ik}$ entonces
...
8- El valor de un determinante no varía si a todos los elementos de una de sus columnas se suman los elementos de otra 
...
Una matriz cuadrada se llama _semidescompuesta_ _en_ _bloques_ si sus elementos pueden ser divididos mediante una línea vertical y otra horizontal en cuatro matrices de manera que a lo largo de la diagonal figuran matrices **cuadradas** y una de las otras dos matrices esté compuesta integramente por ceros. Es decir, la matriz $A$ es _semidescompuesta_ _en_ _bloques_ si tiene una de las dos formas siguientes
...
\begin{equation}
  ...
  \end{vmatrix}
\end{equation}
10.- El determinante de una matriz semidescompuesta en bloques es igual al producto de los determinantes de sus bloques diagonales. Es decir 
Si
\begin{equation}
  \begin{vmatrix}
     \mathbf{A}       &\mathbf{B}    \\ 
     \mathbf{O}       &\mathbf{C} 
      \end{vmatrix}
\end{equation}
Entonces el determinante de la matriz es igual a $|\mathbf{A}||\mathbf{C}|$
