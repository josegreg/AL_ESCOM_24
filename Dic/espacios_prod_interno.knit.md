...
<strong>Definición</strong>
	Un espacio vectorial $V$ en el que hay definido un producto interno se denomina **espacio (vectorial real) con producto interno**.
</div>


Si por el contrario, tenemos un espacio vectorial complejo, es decir $\mathbb{K}=\mathbb{C}$, un <strong>producto interno</strong> $V$ es una función que a cada par de vectores 
	$\mathbf{u}$ y $\mathbf{v}$ en $V$, le asigna un número **complejo** denotado por $\langle \mathbf{u}, \mathbf{v}\rangle$.
	Esta función satisface las siguientes propiedades: si $\mathbf{u}, \mathbf{v}$ y $\mathbf{w}$ son vectores y 
	$c$ es un escalar, entonces
	
	
$a)$ $\langle \mathbf{v}, \mathbf{u}\rangle = \overline{\langle \mathbf{u}, \mathbf{v}\rangle}$. 

$b)$ $\langle \mathbf{u}, \mathbf{v}+\mathbf{w}\rangle 
			      = \langle \mathbf{u}, \mathbf{v}\rangle + \langle \mathbf{u}, \mathbf{w}\rangle$. 
			      
$c)$ $\langle c\mathbf{u}, \mathbf{v}\rangle = c\langle \mathbf{u}, \mathbf{v}\rangle$
...
<div style="background-color:#FFE9B1">
<strong>Definición</strong>
	Un espacio vectorial $V$ en el que hay definido un producto interno se denomina **espacio (vectorial complejo) con producto interno**.
</div>
...
Mostrar que en $V=\mathbb{R}_3[x]$, la siguiente función $\langle , \rangle:V\times V \rightarrow \mathbb{R}$ $\langle p, q\rangle=\int_{-1}^1p(s)q(s)ds$ es un producto interno.
...
a) Para todo $\mathtt{x}\in V$, si $\|\mathtt{x}\|= \mathbf{0} \Leftrightarrow \mathtt{x}=\mathbf{0}$. Si $x\ne \mathbf{0}$ entonces $\|\mathtt{x}\| > 0$
...
Sea $U$ un conjunto de vectores en $V$, un espacio con producto interno.
Definimos al complemento ortogonal de $U$ como
...
Si $U=\{\mathtt{v}_1,\mathtt{v}_2,\ldots, \mathtt{v}_k\}$ son ortogonales y $\mathtt{v}\in V$ entonces
\[
...
@. Sea $V=\mathbb{R}^4$ y $\left \langle \mathtt{x}, \mathtt{y}\right \rangle = \mathtt{x} \cdot \mathtt{y}$, el producto punto. Sea 
\[
\mathtt{\vec{x}} =\begin{pmatrix} -4 \\ 2 \\ 1 \\2 \end{pmatrix}, \quad\quad \mathtt{\vec{y}} =\begin{pmatrix} 1 \\ 0 \\ 2 \\2 \end{pmatrix}
\]
Calcula el ángulo entre los vectores $\mathtt{\vec{x}}$ y $\mathtt{\vec{y}}$.
 ...
 * Escribir a $\vec{x}=\begin{pmatrix} -1 \\ 2 \\ 1 \end{pmatrix}$ como combinación lineal de $\mathcal{B}$.
 
@. Sea $\vec{v}=(6,7)\in\mathbb{R}^2$ y $\left \langle \vec{x}, \vec{y} \right \rangle_B = 2x_1y_1 -3x_1y_2-3x_2y_1 + 2x_2y_2$, calcular 

 * Si $\mathtt{e}_1 = (1,0)$ y $\mathtt{e}_2 = (0,1)$, calcular $\left \langle \mathtt{e}_1, \mathtt{e}_2 \right \rangle_B$, $\|\mathtt{e}_1\|_B$,  $\|\mathtt{e}_2\|_B$.
 
 * Calcular los cosenos directores de $\vec{v}=(6,7)$ con respecto a ese producto interno.
 
@. Sea $\mathtt{u}_1=\begin{pmatrix} 1 \\ 0 \\ 0 \end{pmatrix},\,$ $\mathtt{u}_2=\begin{pmatrix} 1 \\ 1 \\ 0 \end{pmatrix},\,$ $\mathtt{u}_3=\begin{pmatrix} 1 \\ 1 \\ 1 \end{pmatrix}$ .

 * Obtener un conjunto de vectores ortogonales, aplicando Gram-Schmidt.
 
 * Obtener una base ortonormal.
 
@. Determinar si $\mathcal{B} = \{\mathtt{u}=(1,2,1)$, $\mathtt{v}=(1,-1,1)$ y $\mathtt{w}=(-1,1,2) \}$.

 * Calcular las normas de los vectores con respecto a la norma inducida por el producto punto.
